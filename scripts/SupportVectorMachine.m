...
%[Epoch.Data,TimeSegmentation] = AdaptiveSegmentation(Temp,NumSegmentation);
load ./Segmentation.mat

H = CSP(Segmentation{2},Segmentation{1});
H = [H(:,1:2) H(:,end-1:end)];

X = [];
for i=1:NumTrainingData(1)
    X = [X; reshape(Segmentation{1}(:,:,i),1,size(Segmentation{1},1)*size(H,2)) FilteredVariance(H,Segmentation{1}(:,:,i))];
end

for i=1:NumTrainingData(2)
    X = [X; reshape(Segmentation{2}(:,:,i),1,size(Segmentation{2},1)*size(H,2)) FilteredVariance(H,Segmentation{2}(:,:,i))];
end

TestData = [];
for i=NumTrainingData(1)+1:NumTrainingData(1)+NumTestData(1)
    TestData = [TestData; reshape(Segmentation{1}(:,:,i),1,size(Segmentation{1},1)*size(H,2)) FilteredVariance(H,Segmentation{1}(:,:,i))];
end

for i=NumTrainingData(2)+1:NumTrainingData(2)+NumTestData(2)
    TestData = [TestData; reshape(Segmentation{2}(:,:,i),1,size(Segmentation{2},1)*size(H,2)) FilteredVariance(H,Segmentation{2}(:,:,i))];
end
